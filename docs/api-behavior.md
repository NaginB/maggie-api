...
When `settings.softDelete` is configured, DELETE sets the configured deletion timestamp (and optional actor field) instead of removing the document. Soft-deleted documents return `404` from generated by-id routes and are excluded from list routes.
